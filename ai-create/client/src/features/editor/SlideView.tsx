...
export function SlideView({ slide, content }: { slide: Slide; content: PresentationContent }) {
  ...
  return <div className="slide-frame"><article className="designed-slide" aria-label={`Предпросмотр: ${slide.title}`} style={{ background: '#' + (cover ? palette.cover : palette.background), color, fontFamily: design.font, textAlign: design.align }}>
    {slide.image && <div style={{ ...position(imageRect(slide.image)), opacity: slide.image.opacity }}>{url ? <img className="slide-raster" src={url} alt={slide.image.alt} style={{ objectFit: slide.image.fit }} /> : <div className="image-placeholder">{failed ? 'Изображение недоступно' : 'Загрузка изображения…'}</div>}</div>}
  ...
  </article>{blocks.some(item => item.fitted.overflow) && <p role="status" className="notice error">Текст не помещается: сократите его или измените положение картинки.</p>}</div>
}
