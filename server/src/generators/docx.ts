import { Document, Packer, Paragraph, TextRun, HeadingLevel, Table, TableRow, TableCell, WidthType, PageBreak, LevelFormat, AlignmentType, Footer, PageNumber, BorderStyle } from 'docx'
import type { SavedDocument, TextBlock } from '../contracts/document.js'
import { HttpError } from '../middleware/errors.js'

export async function generateDocx(document: SavedDocument): Promise<Buffer> {
  ...
  const file = new Document({ creator: 'Forma', title: document.title, styles: { default: { document: { run: { font: 'Arial', size: 22, color: '000000' }, paragraph: { spacing: { line: 300, after: 160 } } } }, paragraphStyles: [
