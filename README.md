...
Это локальная версия с редакторами, экспортом Office и AI-генерацией презентаций. По текущему решению владельца по умолчанию используется Gemini с проектом Free Tier без Billing. Генерация картинок в этом режиме отключена; загрузка своих изображений работает. OpenAI доступен только при явном выборе AI_PROVIDER=openai и оплачивается отдельно. Production deployment ещё не выполнен.
...
| `GEMINI_MODEL` | gemini-3.8-flash по умолчанию; нужна доступная квота модели в проекте |
...
- AI-интеграция реализована для презентаций и изображений. Реальные вызовы не проверены: локальный OPENAI_API_KEY не заполнен. Ответы, отказы и ошибки проверены через подменённый транспорт. AI для Word/Excel и правка выбранного блока — следующие этапы.
...
- Полная тёмная тема, шаблоны, настройки, landing, CI и deployment — последующие этапы.
...
- В исходном Git отслеживается `server/node_modules`. Ранее запрос на изменение индекса был отклонён; он не повторялся. `.gitignore` исправлен, но старое отслеживание остаётся. Установка зависимостей поэтому видна в Git. Отдельное исправление: `git rm -r --cached -- server/node_modules` (сохраняет локальные файлы).
- Commit, push, публикация и изменения production-БД не выполнялись.
